    ...
    getAttributeModifiers(name, attributeName) {
        switch(this.type){
            ...
            case ContextTypes1.ComponentDeclaration:
                ...
                if (node) {
                    const modifier1 = node.getAttributeModifiers(name, attributeName);
                    return modifier1;
                } else return null;
            default:
                return null;
...
function readStyleSheetConstAtRuleCtx(exium, opts1) {
    ...
    try {
        const { x ,  } = exium.cursor;
        const { source: source2  } = exium;
        const isValid = Boolean(isFollowedBy(exium, "@const", true) || opts1?.data?.isExportStatement);
        if (!isValid) return isValid;
        if (opts1?.checkOnly) return true;
        ...
        const token = source2.slice(x, exium.cursor.x);
        const context = new ExiumContext1(ContextTypes1.StyleSheetAtRuleConst, token, x);
        context.children.push(...children);
        context.related.push(...related);
        exium.currentContexts.push(context);
        return true;
...
function readStyleSheetTypeAssignementCtx(exium, opts1) {
    ...
    try {
        ...
        const { x ,  } = exium.cursor;
        const { source: source2  } = exium;
        const isValid = Boolean(prev === "@" && __char === "<" || opts1?.data?.force_type_assignment_context);
        if (!isValid) return isValid;
        if (opts1?.checkOnly) return true;
        const result = true;
        let isClosed = false;
