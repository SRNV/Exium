...
(function(Reason1) {
    ...
    Reason1[Reason1["StyleSheetAtRuleCurlyBracesAreMissing"] = 2960] = "StyleSheetAtRuleCurlyBracesAreMissing";
...
class ExiumBase {
    ...
    get lastContext() {
        const last = this.currentContexts[this.currentContexts.length - 1] || this.unexpected;
        return last;
    }
    ...
    string_template_quote_eval_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            if (__char !== "$" || __char === "$" && prev === '\\' || __char === "$" && next !== '{') return false;
            if (opts?.checkOnly) return true;
            let result = true;
            let isClosed = false;
            const allSubContexts = [
...
class ExiumHTMLElements extends ExiumBase {
    ...
    isStartingNode() {
        return [
            '<', 
        ].includes(this.char) && (this.node_CTX(this.checkOnlyOptions) || this.html_comment_CTX(this.checkOnlyOptions));
    }
    textnode_CTX(opts) {
        try {
            ...
            const isValid = prev && [
                '>'
            ].includes(prev) && lastIsANode || __char !== '<' && !this.import_statements_CTX(this.checkOnlyOptions) && !this.node_CTX(this.checkOnlyOptions) && !this.comment_CTX(this.checkOnlyOptions);
    ...
    node_CTX(opts) {
        try {
            ...
            if (__char !== "<" || __char === "<" && [
                ...
                '<',
                '!'
            ].includes(next) || next && /([^a-zA-Z0-9\[\/])/i.test(next)) return false;
            if (opts?.checkOnly) return true;
            let result = true;
            let isClosed = false;
            ...
            const token = source2.slice(x, this.cursor.x);
            const context = new ExiumContext(isNodeClosing ? ContextTypes.NodeClosing : ContextTypes.Node, token, {
                start: x,
                end: this.cursor.x,
                line,
                column
            ...
            if (!isAutoClosing) {
                if (isClosed && !isNodeClosing) {
                    ...
                    const openTag = this.openTags.slice().reverse().find((nodeContext)=>{
                        const name = nodeContext.related.find((related1)=>related1.type === ContextTypes.NodeName
                        );
                        const targetName = context.related.find((related1)=>related1.type === ContextTypes.NodeName
                        );
                        return name && targetName && !nodeContext.data.closed && name.type === ContextTypes.NodeName && name.source === targetName.source;
                    });
                    if (!openTag) {
    ...
    html_comment_CTX(opts) {
        try {
            ...
            while(!this.isEOF){
                ...
                if (this.char === ">" && this.prev === '-' && source2[this.cursor.x - 2] === '-') {
                    this.shift(1);
                    isClosed = true;
                    break;
                }
    ...
    import_statements_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            const sequence = __char + next + source2[x + 2] + source2[x + 3] + source2[x + 4] + source2[x + 5] + source2[x + 6];
            ...
            while(!this.isEOF){
                this.shift(1);
                this.isValidChar(opts?.unexpected);
                const sequenceEnd = this.char + this.next + source2[this.cursor.x + 2] + source2[this.cursor.x + 3];
                if (sequenceEnd === 'from') {
                    this.cursor.x += +4;
    ...
    flag_CTX(opts) {
        try {
            ...
            while(!this.isEOF){
                ...
                this.isValidChar(opts?.unexpected);
                if (!isNamed) {
                    isNamed = Boolean(this.flag_name_CTX() && related.push(this.lastContext));
                }
                this.saveContextsTo(allSubContexts, children);
                if ([
                    ' ',
    ...
    flag_spread_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            if (__char !== '{' || !/^\{(\s*)(\.){3}/i.test(this.nextPart)) return false;
            if (opts?.checkOnly) return true;
            let result = true;
            let isClosed = false;
            const children = [];
    ...
    attributes_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            if (prev && prev !== ' ' || __char && !/[a-zA-Z0-9\$\_]/i.test(__char)) return false;
            if (opts?.checkOnly) return true;
            let result = true;
            let isClosed = false;
            let isNamed = false;
            ...
            if (!isNamed) {
                isNamed = Boolean(this.attribute_name_CTX() && related.push(this.lastContext));
            }
            while(!this.isEOF){
                this.shift(1);
                this.isValidChar(opts?.unexpected);
    ...
    attribute_unquoted_CTX(opts) {
        try {
            ...
            const token = source2.slice(x, this.cursor.x);
            const context = new ExiumContext(ContextTypes.AttributeValueUnquoted, token, {
                start: x,
                end: this.cursor.x,
                line,
                column
...
class ExiumProtocol extends ExiumHTMLElements {
    ...
    protocol_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            const lastIsAStyleNode = this.currentContexts.find((context)=>context.type === ContextTypes.Node && context.related.find((node)=>node.type === ContextTypes.NodeName && node.source === 'proto'
...
class ExiumStyleSheet extends ExiumProtocol {
    ...
    stylesheet_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            const lastIsAStyleNode = this.currentContexts.find((context)=>context.type === ContextTypes.Node && context.related.find((node)=>node.type === ContextTypes.NodeName && node.source === 'style'
                ) && !context.related.find((node)=>node.type === ContextTypes.NodeClosing
                )
            );
    ...
    stylesheet_const_at_rule_CTX(opts) {
        try {
            ...
            const isValid = Boolean((prev === '@' || opts?.data?.isExportStatement) && sequence === 'const ');
            if (!isValid) return false;
            if (opts?.checkOnly) return true;
            let result = true;
            let isNamed = false;
    ...
    stylesheet_default_at_rule_CTX(opts) {
        try {
            ...
            while(!this.isEOF){
                ...
                this.isValidChar(opts?.unexpected);
                this.saveContextsTo(allSubContexts, children);
                if (this.char === '{' || this.char === ';' || this.isEndOfStylesheet()) {
                    break;
                }
            }
    ...
    stylesheet_type_assignment_CTX(opts) {
        try {
            ...
            const { x , line , column  } = this.cursor;
            let { source: source2  } = this;
            const isValid = __char === '<' && (prev === '@' || opts?.data?.force_type_assignment_context);
            if (!isValid) return false;
            if (opts?.checkOnly) return true;
            let result = true;
            let isClosed = false;
